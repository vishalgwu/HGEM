...
the schema layer knows nothing about.** `grammar_safe` is that seam - see it
    ...
    async def _one(
        ...
        **`offset` is why the K samples differ, and leaving it out silently
        ...
        if schema is not None:
            payload["format"] = grammar_safe(schema.model_json_schema())
        try:
            ...
            # Before `httpx.HTTPError`, which it subclasses: a timeout means
            # the server answered and is still generating, so the branch below
            # would point at an `ollama serve` already running. `ReadTimeout`
            # also stringifies to "", so it read "unreachable ... : ." - blank
            # cause, wrong remedy. CHECKPOINT B lost a conversation to it.
            raise ProviderUnavailable(
                f"ollama timed out for {model} after {self._timeout_s}s at "
                f"{self._client.base_url}. The server is answering; this prompt "
                "did not finish in the budget. Raise `GM_OLLAMA_TIMEOUT_S` or "
                "shorten the input."
            ) from exc
