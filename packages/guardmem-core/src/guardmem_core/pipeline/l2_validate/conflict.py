"""Conflict detection.  BUILD_NOTEBOOK.md S4.2 (retrieval), S4.3 (the checks)

`MEMORY_ENGINE.md` §2.2 in two halves, built in two steps. This is the first:
"Retrieve incumbents: top-k (k=10) by cosine within `(namespace, subject,
predicate)` plus graph neighbors 1 hop out." The three checks that compare a
candidate against what comes back - NLI contradiction, cardinality, temporal
overlap - are S4.3 and land in this module beside it.
...
class IncumbentSet(GMModel):
    ...
    Attributes:
        ...
        nearest: Up to `k` live assertions sharing the candidate's namespace,
            subject and predicate, nearest first by cosine. These are what
            S4.3's three checks run against - each carries its provenance, and
            the NLI pass compares `incumbent.provenance[*].verbatim` with the
            candidate's.
...
def _widening(edges: list[Edge], nearest: list[StoredAssertion]) -> list[Edge]:
    ...
    shape.
    """
    seen = {assertion.assertion_id for assertion in nearest}
