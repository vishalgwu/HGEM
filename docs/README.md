...
| [Architecture decisions](adr/) | Rationale for dual stores, bitemporal history, async evaluation, entropy, MCP, the Day-2 amendments to `ExtractionResult` and `Provenance`, **entity resolution — which binds and never matches**, **`pii_class` and `irreversibility` as ontology fields**, and **the applier's transaction boundary**, and **the pool's bound on waiting for a connection** (0011 is reserved for `memory.commit`) |
