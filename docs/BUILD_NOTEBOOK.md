...
**As of 2026-09-15 every dependency is implemented and `run()` runs.** Verified
end to end against a real Postgres and a real local model: one candidate in, one
`HITL_REVIEW` out at `C = 0.837`, `R = 0.924`, with a real entity row written
through the assertion's foreign key.

- `EntailFn` -> `llm/entailment.py` (S5.1 correction 4).
- `EntityResolver` -> `memory/entities.py` (ADR-0008).
- `CandidateClassifier` -> **deleted** rather than implemented (ADR-0009). Two of
  its three features are now `PredicateSpec` fields and the third always came
  off the namespace.

**Generation shipped too**: `scripts/checkpoint_b_generate.py` fills a corpus
from a proposals file, and `generate` -> label -> `score` has been run end to
end. So the harness is complete.

What is left is a **corpus** and **labels**, and neither is code. Step 1 says
"200 candidates from the seed transcript"; that transcript is forty turns for
one patient supporting 28 facts, so 200 needs more conversations - short ones,
because a 40-turn transcript times out at a 600-second per-request ceiling on a
local model.

**This gate needs no API key.** `GM_ANTHROPIC_API_KEY` has been blank since S0.2
and a local Ollama model runs the whole pipeline for nothing - verified, after
the Ollama grammar fix: `ExtractedFact.verbatim` carries `maxLength: 2000`,
Ollama compiles `format` into a grammar where a bounded length is a repetition,
and repetitions of 2000 or more are refused outright. Bisected against Ollama
0.34.0 with `llama3.1:8b`: **1999 compiles, 2000 does not**. The adapter strips
those keywords from the grammar it sends and the caller still validates the
reply against the full schema, so the cap moves from prevention to detection.
